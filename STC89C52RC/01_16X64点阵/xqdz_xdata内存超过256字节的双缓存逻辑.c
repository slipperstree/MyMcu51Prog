...
void main()
{
	uchar bufNumBackground;
	Timer0Init();
	
	// For test --------------------------------------------------------------------------------
	// setICDataToBuffer(nowBufferNum, getICData_Hanzi_16x16("全"), 32, 0);
	// setICDataToBuffer(nowBufferNum, getICData_Hanzi_16x16("场"), 32, 2);
	// setICDataToBuffer(nowBufferNum, getICData_Hanzi_16x16("两"), 32, 4);
	// setICDataToBuffer(nowBufferNum, getICData_Hanzi_16x16("折"), 32, 6);
	//setICDataToBuffer(nowBufferNum, getICData_Hanzi_16x16("语"), 32, 5);
	...
	while (1){
		// if(isReadingIC == 1){
			...
			// // 只要开始显示旧数据就立刻开始读下一组汉字的IC信息到另外一个buffer中备用
			// if(nowBufferNum == 0){
				// bufNumBackground = 1;
			// } else {
				// bufNumBackground = 0;
			// }
			// readICDataToBuffer(bufNumBackground, &showData[nowPos]);
		// }
...
void timer0() interrupt 1
{
	...
	if(tt >= 300){
		...
		if(nowOffset==15){
			...
			//为了避免读新的汉字数据占用大量时间导致显示时间不均匀整个屏幕会闪烁的问题，读取ic数据的工作放在主循环里做
			//只要保证位移16位的时间内读完就可以了，足够了。
			
			//这里只做位移量的自加和下一个汉字位置的自加（主循环里的读ic函数会用这个pos信息）
			//nowPos+=2;
